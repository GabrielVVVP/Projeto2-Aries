...
def menu(request,user_id):
    if request.method == 'POST':
        ...
        return render(request, 'stations/index4.html', {'stations': all_stations, 'parameters':all_params, 'user_id':user_id})
...
def check_data(request,user_id,station,name,type_chart='line'):
    if request.method == 'GET':
        ...
        if (dates != ""):
            ...
            dates_list = [""]  
        return render(request, 'stations/index2.html', {'station': check_station, 'parameter':check_parameter, 'values':values_list, 'dates':dates_list, 'type':type_chart, 'user_id':user_id})
    else:
        return redirect(reverse('menu', kwargs={"user_id": user_id}))
...
def check_location(request,user_id,station,name):
    ...
    return render(request, 'stations/index3.html', {'station': check_station, 'parameter': check_parameter,'coordinates':coordinates, 'key':api_key, 'user_id':user_id})

def subscribe(request,user_id,station,name):
    if request.method == 'GET':
        check_station = Station.objects.all().filter(name=station).first()
        check_parameter = Parameter.objects.all().filter(name=name,station=check_station).first()
        return render(request, 'stations/index6.html', {'station': check_station, 'parameter': check_parameter, 'user_id':user_id})  
    else:
        check_station = Station.objects.all().filter(name=station).first()
        check_parameter = Parameter.objects.all().filter(name=name,station=check_station).first()
